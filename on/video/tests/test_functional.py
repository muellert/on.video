...
class TestOnVideoHandling(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for d in self.td, subdir:
            for video in videofiles:
                ...
                o.write("1")
                o.close()
        settings.fspath = unicode(self.td)
        #print "\n ====> TestOnVideoHandling: fspath = ", settings.fspath
    ...
    def test_add_video_via_browser_no_metadata(self):
    ...
    # below: test for the three required fields
    # title, filename, and body

    def test_add_video_without_title(self):
        """Fill out the video 'add' form without giving a title, so
        ...
        save_button.click()
        self.failUnless("Required input is missing." in browser.contents)

    def test_add_video_without_metadatafile(self):
        ...
        body_field = browser.getControl(name='form.widgets.body')
        save_button = browser.getControl(name='form.buttons.save')
        title_field.value = u'Hi there!'
        body_field.value = u'<p>nothing to see here, move along</p>'
        save_button.click()
        self.failUnless("Required input is missing." in browser.contents)

    def test_add_video_without_body(self):
        """Fill out the video 'add' form with the wrong date and notice
           that it does not validate.
        """
        app = self.layer['app']
    ...
    ### Disable this test for now until we figure out why the fspath registry
    ### entry is not being properly set up by the setUp() method (it returns
    ### u'/tmp' in all cases, instead of the generated temp dir, causing this
    ### test to fail):
    def XXXtest_add_video_with_missing_thumbnail_but_thumbnail_line_in_metadata(self):
        """Now try sample_metadata_1 to verify that the thumbnail is
        ...
        save_button = browser.getControl(name='form.buttons.save')
        title_field.value = u'Hi there!'
        filename_field.value = u'sample_video_1'
        body_field.value = u'<p>nothing to see here, move along</p>'
        save_button.click()
        import pdb; pdb.set_trace()
        self.failUnless("nothumbnail" in browser.contents)
...
# need to check for bogus dates someday, too:
